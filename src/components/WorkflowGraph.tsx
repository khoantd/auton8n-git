...
const WorkflowGraph: React.FC<WorkflowGraphProps> = ({ workflow }) => {
  ...
  const workflowJsonString = React.useMemo(() => {
    try {
      ...
      if (!validation.valid) {
        setErrorMessage(validation.error || 'Invalid workflow');
        return '';
      }
  ...
  useEffect(() => {
    ...
    try {
      // Force the component to fill the container
      ...
      // Add resize observer to ensure component fills container
  ...
  return (
    <div className="w-full h-full bg-gradient-to-br from-[#0e1117] via-[#0d1117] to-[#0e1117] overflow-hidden relative border border-slate-800/50 rounded-2xl shadow-2xl shadow-black/40 flex flex-col group">
      <div className="flex-1 min-h-0 n8n-viewer-container relative z-10 flex flex-col">
      {/* Warnings display */}
      ...
      {/* Loading state */}
        ...
        {hasError && (
          <div className="absolute inset-0 bg-gradient-to-br from-[#0e1117] to-[#0d1117] flex flex-col items-center justify-center p-8 z-20">
            <div className="bg-gradient-to-br from-red-900/30 to-red-950/20 border border-red-900/40 rounded-2xl p-6 mb-6 max-w-md shadow-2xl shadow-red-900/20 backdrop-blur-sm">
